    ...
    <v-list nav :style="navStyle">
      ...
      >
        <template v-slot:activator="{ props }">
          <v-list-item
            v-bind="props"
            :prepend-avatar="menuItem?.icon_url"
            class="mb-3"
            :style="
              getActiveMenu == menuItem.id ? activeItemStyle : inActiveItemStyle
            "
        ...
        <v-list-group
          expand-icon=""
          collapse-icon=""
          v-if="menuItem.sub_menu_items.length"
        ...
        >
          <template v-slot:activator="{ props }">
            <v-list-item
              v-bind="props"
              :style="
                getActiveMenu == subItem.id
                  ? activeItemStyle
                  : inActiveItemStyle
              "
              :prepend-avatar="subItem.icon_url"
              @click="onClickSubMenu(index, subIndex)"
            >
              <v-list-item-content>
  ...
  export default {
    ...
    methods: {
      ...
      onClickParentMenu(index) {
        ...
        if (
          !MainMenuTypes[this.$store.state.MenuStore.MenuIds[index]]
            .sub_menu_items.length
        ) {
          let menuId = MainMenuTypes.length
            ? MainMenuTypes[this.$store.state.MenuStore.MenuIds[index]].id
            : []
      ...
      onClickSubMenu(index, subIndex) {
        ...
        if (
          ...
          let subMenuId = MainMenuTypes.length
            ? MainMenuTypes[this.$store.state.MenuStore.MenuIds[index]]
                .sub_menu_items[subIndex].id
            : []
          let menuId = MainMenuTypes.length
            ? MainMenuTypes[this.$store.state.MenuStore.MenuIds[index]].id
            : []
    ...
    computed: {
      ...
      showMenuBar() {
        this.$store.state.NavigationStore.navigationState
          ? (this.isOverLeave = true)
          : (this.isOverLeave = false)

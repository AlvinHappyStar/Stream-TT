<template>
  <v-container fluid style="padding: 0px" @click="navigationStateChange()">
    <div v-if="phoneWidth <= 561">
      <v-row no-gutters>
        <div
          ...
          :style="{
            'background-image':
              'url(' +
              (isDev ? SliderImage : encodeURI(SVOD?.wide_poster_url)) +
              ')',
          }"
          style="
      ...
      <v-row no-gutters >
        <v-col
          v-if="SVOD?.length"
          class="d-flex justify-flex-center"
          :style="SidebarStyle"
          style="width: 100vw; background: none"
        >
          <div
            style="
              display: flex;
          ...
          >
            <v-btn
              class="d-flex justify-flex-end align-center pr-4 mb-6 font-weight-medium text-h6"
              v-if="SVOD?.package_status == false"
              style="background: hsl(242deg 2% 5%)"
              @click="onShowAccessOptions()"
            >
              <v-icon
                style="
                  font-size: 34px;
            ...
            <div
              class="d-flex flex-column justify-flex-left align-left px-5 py-5"
            >
              <div class="font-weight-bold text-h6 text-left mb-4">
                {{ SVOD?.name_short }}
              </div>
              <div
                class="font-weight-medium text-subtitle-2 text-left pl-1 mb-4"
              >
                {{ SVOD?.description }}
              </div>

              <div class="d-flex justify-between align-start pl-1 mb-2">
                <div
                  class="text-left font-weight-medium text-subtitle-2"
                  style="max-width: 90px; flex: 1"
                >
                  Directors:
                </div>
                <div class="text-left font-weight-medium text-subtitle-2">
                  {{ SVOD?.director }}
                </div>
              </div>
              ...
              <div class="d-flex justify-between align-start pl-1 mb-2">
                ...
                <div
                  class="font-weight-medium text-left text-subtitle-2"
                  style="flex: 1"
                >
                  {{ SVOD?.actors }}
                </div>
              ...
              <div class="d-flex justify-between align-start pl-1 mb-2">
                ...
                <div class="font-weight-medium text-left text-subtitle-2">
                  {{ this.setRuntime(SVOD?.duration) }}
                </div>
              </div>
              <div class="d-flex justify-between align-start pl-1 mb-2">
                <div
                  class="font-weight-medium align-start text-left text-subtitle-2 pl-1 mb-2"
                  style="max-width: 90px; flex: 1"
                >
                  Year:
                </div>
              ...
              <div class="pl-1 mt-4 d-flex justify-starter align-start">
                <img
                  :src="SVOD?.content_provider_logo"
                  style="
                    cursor: pointer;
                    width: 60px;
                    height: 60px;
                ...
                />
                <div
                  class="content-provider-name font-weight-medium text-h6 ml-2 mr-5"
                  @click="goToProviderPage(SVOD?.content_provider_id)"
                  style="cursor: pointer"
                >
                  {{ SVOD?.content_provider_name }}
                </div>

                <div
                  class="d-flex flex-column pr-4 ml-md-16"
                  v-if="SVOD?.package_status == true"
                >
                  <v-btn
      ...
      >
        <v-row no-gutters>
          ...
          >
            <div>
              <div
                class="d-flex flex-column justify-start align-start pl-16 mb-6"
              >
                <div class="font-weight-medium text-h3 pl-10 mb-10">
                  {{ SVOD?.name_short }}
                </div>
                <div
                  class="font-weight-medium text-subtitle-2 text-left pl-10 mb-10"
                >
                  {{ SVOD?.description }}
                </div>
                ...
                <div class="font-weight-medium text-subtitle-2 pl-10 mb-2">
                  Year: {{ SVOD?.year }}
                </div>
                <div class="pl-10 mt-16 d-flex justify-starter align-center">
                  ...
                  <v-btn
                    class="d-flex justify-flex-end align-center pr-4 ml-16 font-weight-medium text-h6"
                    v-if="SVOD?.package_status == false"
                    style="background: hsl(242deg 2% 5%)"
                    @click="onShowAccessOptions()"
                  >
  ...
  <v-dialog v-model="dialog" max-width="450" hide-overlay="true">
    <v-card class="rounded-lg">
      <v-card-title class="text-h5 text-center"> Access Options </v-card-title>
      <v-list class="mx-auto" dense>
        <v-list-item-group v-model="selectedItem" color="primary">
          ...
          >
            <v-divider
              v-if="index < accessOptions.length"
              :key="index"
            ></v-divider>
  ...
  <v-dialog v-model="signDialog" persistent width="auto">
    <v-card>
      ...
      <v-card-text
        >Please sign in to view the content or to see what pacakges are
      ...
      return {
        ...
        SidebarStyle: {
          ...
          background:
            ...
            ' 50%, ' +
            SIDEEFFECTBAR_3 +
            ' 76%, transparent 89%, transparent 100%)',
          minHeight: '100vh',
        },
      }
    },
    ...
    computed: {
      ...
      SVOD() {
        ...
        if (svdCnt > 0 && svdCnt >= this.vodId) {
          this.accessOptions =
            this.$store.state.SVODStore.SVOD[this.vodId].packages
          console.log(
            '>>>>>>>>>>>>>>>>>>>>>',
            this.$store.state.SVODStore.SVOD[this.vodId].movie_info[0]
          )

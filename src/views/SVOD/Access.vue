        ...
        <div v-if="phoneWidth <= 561">
            <v-row no-gutters>
                <div v-if="SVOD" :style="{ 'background-image': 'url(' + (isDev?SliderImage:encodeURI(SVOD?.wide_poster_url)) + ')' }" 
                    style="background-repeat: no-repeat; background-attachment:local; background-position:center; background-size:cover; width: 100%; height:500px">
                </div>
            </v-row>
            <v-row no-gutters>
                
                <v-col v-if="SVOD?.length" class="d-flex justify-flex-center" :style="SidebarStyle" style="width:100vw;background:none;">
                    <div style="display:flex;align-items:center;flex-direction:column">
                        <v-btn class="d-flex justify-flex-end align-center pr-4 mb-16 font-weight-medium text-h6" v-if="SVOD?.package_status == false"
...
export default {
    ...
    data() {
        console.log("st", window.innerWidth)
        return {

  ...
  <v-container fluid style="padding: 0px" @click="navigationStateChange()">
    <div v-if="phoneWidth <= 561">
      <div
        v-if="SVOD"
        :style="{
          'background-image': 'url(' + encodeURI(SVOD?.backdrop) + ')',
        }"
        style="
          ...
          >
            <div class="d-flex flex-column justify-start align-start px-5 py-5">
              ...
              <div class="pl-3 pr-2 mt-2 d-flex justify-starter align-center">
                ...
                />
                <div
                  class="content-provider-name font-weight-medium text-h6 ml-2"
                  @click="goToProviderPage(SVOD?.content_provider_id)"
            ...
            <div
              @click="episodeClick(SVOD?.package_status, episode.id)"
              v-for="(episode, i) of episodes"
            ...
            >
              <div class="d-flex flex-column justify-center align-center">
                <div class="d-flex justify-start align-start">
                  <img
                    :src="episode.icon_url"
                    style="width: 30vw; border-radius: 25px"
                  />
                  <div class="font-weight-medium text-h5 pl-5 mb-5">
                    {{ episode.caption }}
                  </div>
                </div>
    ...
    <div v-else>
      ...
      >
        <v-row no-gutters>
          <v-col
            cols="6"
            class="d-flex justify-center left-side-content pl-2"
            :style="SidebarStyle"
            style="padding-top: 150px"
          >
            <div
              class="d-flex flex-column justify-start align-start pl-2 mb-6"
              style="width: 40vw"
            >
              ...
              <div
                class="font-weight-medium text-subtitle-2 text-left pl-5 mb-10"
              >
                {{ SVOD?.description }}
              </div>
              ...
              <div
                class="d-flex justify-starter align-center ml-10 mt-16"
          ...
          <v-col
            cols="6"
            class="d-flex flex-column align-center pr-16 pl-10"
            style="padding-top: 150px; height: 100vh; overflow: auto"
            v-if="episodes.length > 0"
          >
            <div
              @click="episodeClick(SVOD?.package_status, episode.id)"
              v-for="(episode, i) of episodes"
              class="episode-item d-flex justify-center align-center px-5 py-5 mb-10"
              :style="SubItemStyle"
            >
              <img
                :src="episode.icon_url"
                style="width: 14vw; border-radius: 25px"
              />
              <div class="d-flex flex-column justify-start align-start">
                <div class="font-weight-medium text-h5 pl-10 mb-5">
                  {{ episode.caption }}
                </div>
                <div class="font-weight-medium text-subtitle-2 text-left pl-10">
                  {{ episode.description }}
        ...
        <div class="d-flex flex-column mb-2">
          <v-btn
            ...
            @click="
              showTrailerModal(
                selected_episode.id,
                selected_episode.vod_trailer_ott_url
              )
            "
            rounded="lg"
  ...
  export default {
    ...
    methods: {
      ...
      resumeMovie(id, resume) {
        this.$router.push({
          path: '/player/series/' + id + '&resume=' + resume,
        })
      },
      moviePlay(id) {
    ...
    computed: {
      ...
      SVOD() {
        ...
        if (svdCnt > 0 && svdCnt >= this.vodId) {
          try {
            ...
            this.series_caption =
              this.$store.state.SVODStore.SVOD[this.vodId].series.name_short
      ...
      seasons() {
        ...
        if (svdCnt > 0 && svdCnt >= this.vodId) {
          try {
            let seasonArray =
              this.$store.state.SVODStore.SVOD[this.vodId].series.seasons
